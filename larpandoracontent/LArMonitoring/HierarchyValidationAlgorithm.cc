...
{
    ++m_event;
    std::cout << "Running Event: " << m_event << std::endl;
    const CaloHitList *pCaloHitList(nullptr);
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetList(*this, m_caloHitListName, pCaloHitList));
    const MCParticleList *pMCParticleList(nullptr);
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetCurrentList(*this, pMCParticleList));
    const PfoList *pPfoList(nullptr);
    std::cout << " List Name: " <<  m_pfoListName << " List:  " <<  pPfoList << std::endl;
    ...
    LArHierarchyHelper::MCHierarchy::ReconstructabilityCriteria recoCriteria(m_minHits, m_minHitsForGoodView, m_minGoodViews, m_removeNeutrons);
    ...
    if (m_writeTree)
    {
        ...
        for (const MCParticle *const pRoot : rootMCParticles)
        {
            for (const LArHierarchyHelper::MCMatches &matches : matchInfo.GetMatches(pRoot))
            {
                ...
                const float  mcEnergy{pMCNode->GetLeadingMCParticle()->GetEnergy()};
		const int tier{pMCNode->GetHierarchyTier()};
                ...
                float vtxX{0.f}, vtxY{0.f}, vtxZ{0.f};
		for (const LArHierarchyHelper::RecoHierarchy::Node *pRecoNode : nodeVector)
                {
                    ...
                    if (nMatches > 0)
                    {
                        ...
                        vtxX = trueVertex.GetX();
			vtxY = trueVertex.GetY();
			vtxZ = trueVertex.GetZ();
		...
		PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_treename.c_str(), "mcPDG", pdg));
                ...
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_treename.c_str(), "vtxX", vtxX));
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_treename.c_str(), "vtxY", vtxY));
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_treename.c_str(), "vtxZ", vtxZ));
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_treename.c_str(), "vtxDx", vtxDx));
		PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_treename.c_str(), "vtxDy", vtxDy));
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_treename.c_str(), "vtxDz", vtxDz));
                PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_treename.c_str(), "vtxDr", vtxDr));
